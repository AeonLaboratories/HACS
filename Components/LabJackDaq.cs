...
{
	public class LabJackDaq : HacsComponent
	{
		#region Component Implementation

		public static readonly new List<LabJackDaq> List = new List<LabJackDaq>();
		public static new LabJackDaq Find(string name) { return List.Find(x => x?.Name == name); }

		// Digital filters on the Meters need the DAQ scan frequency
		// for their initialization. The scan frequency is obtained 
		// from the DAQ after streaming is underway. So, the DAQ needs to 
		...
		protected void Stop()
        {
            ...
            stopStreaming();
            stateSignal.Set();
            // TODO should we wait here until stopped state is confirmed?
        }

		public LabJackDaq()
		{
			List.Add(this);
			OnPreInitialize += PreInitialize;
			OnStop += Stop;
		}

		#endregion Component Implementation

		// This function is not a Component override. It is intended to be called by
		// the Meter during its Connect() phase.
		public void Connect(Meter meter) { aiMeter.Add(meter); }
		...
		public int OutputPace { get; set; } = 1;        // ms between output signal changes
		[JsonProperty]//, DefaultValue(50)]
		public int OutputIdle { get; set; } = 50;		// ms timeout when nothing to do

		[XmlIgnore] Thread outputsThread;
		[XmlIgnore] AutoResetEvent outputsSignal = new AutoResetEvent(false);
		...
		public int PendingDO => doSetQ.Count;
		...
		public int PendingAO => aoSetQ.Count;
		...
		public int MinimumRetrievalInterval { get; set; } = 40; // ms
		...
		[XmlIgnore] AutoResetEvent streamSignal = new AutoResetEvent(false);

		[XmlIgnore] public bool IsStreaming => streamThread?.IsAlive ?? false;
		...
		public override string ToString()
		{
			...
			sb2.Append(Utility.ToStringLine("AI count", streamData?.Length ?? 0));
		...
		void configureDAQ()
		{
			...
			{
				Notice.Send("Labjack Configuration Error:" + "\r\n" + e.ToString());
		...
		void addToStream(Meter m)
		{
			...
			setVoltageRange(m);
			if (m.AiMode == AiModeType.Differential)
